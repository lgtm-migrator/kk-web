...
const Resume: FC = () => {
  ...
  useEffect(() => {
    ...
    const layout: TableLayout = {
      fillColor: (_, __, columnIndex: number) => (columnIndex % 2 ? null : '#ccf'),
      hLineWidth: (index, { table: { body } }) => (index === 0 || index === body.length ? 1 : 0.5),
      // eslint-disable-next-line max-len
      vLineWidth: (index, { table: { widths } }) => (index === 0 || (Array.isArray(widths) && index === widths.length)
        ? 1
        : 0.5),
    };
    const histories = [
      ...
      (
        ...
        rows.push(
          [
            ...
            {
              colSpan: 3,
              text: `(${(to ? dayjs(to, 'YYYYMM') : dayjs()).diff(
                dayjs(from, 'YYYYMM'),
                'month',
              ) + 1
                }ヶ月間)`,
            ...
            ...[...Array(11)].map(() => ({ text: '' })),
          ].map((cell) => (pageBreakAfter ? { ...cell, pageBreak: 'after' } : cell)),
        );
      },
    );
